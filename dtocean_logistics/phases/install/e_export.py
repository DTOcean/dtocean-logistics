...
e_export.py contains the functions to initiallize the logistic phase: "export cable 
...
landfall (DataFrame): Panda table containing the landfall database
...
def init_e_export_phase(log_op, vessels, equipments, landfall,
    ...
    if len(static_cable) > 0:
        ...
        for index, row in export_db.iterrows():
            index_route_db = export_route[export_route['static cable id [-]'] == index]
            ...
            for a in range(len(index_route_db.index)-1):
                if index_route_db['split pipe [-]'].iloc[a] == 'yes':
                    ...
                    dist = distance(UTM_ini, UTM_fin)*1000  # obtain the distance in meters 
        ...
        phase = LogPhase(100, "Installation of static export cables")
    
        ''' Static Export Cable Installation Strategy '''
...
#            module_logger.warning(msg)
        ...
        for a in range(len(trench_type)): 
            ...
            if technique == 'no burial' or (export_route['burial depth [m]'] == 0).all() or export_route['burial depth [m]'].isnull().all():
            ...
            for index, row in export_db.iterrows():
                ...
                phase.op_ve[a].op_seq_sea[index] = []

                # condition check to obtain the select landfall method
                if landfall_db['method [-]'].iloc[0] == 'OCT':
                    phase.op_ve[a].op_seq_sea[index].extend([ log_op["OCT"] ])

                elif landfall_db['method [-]'].iloc[0] == 'HDD':
                      phase.op_ve[a].op_seq_sea[index].extend([ log_op["HDD"] ])

                else:
                    msg = ("Landfall method not recognised. "
                           "HDD or OCT methods accepted")
                    module_logger.warning(msg)
                ...
                for index1, row in index_route_db.iterrows():
                    ...
                    if index_route_db['burial depth [m]'].ix[index1] == 0 and \
                       ...
                       if trenching == 0:
                        ...
                        msg = ("Error in export cable route: section number "
                               "{} is not valid".format(index1))
                        module_logger.warning(msg)

                # condition check to obtain suitable operation sequence for the offshore termination
                if export_db['upstream termination type [-]'].ix[index] == 'device':

                  if export_db['upstream ei type [-]'].ix[index] == 'dry-mate' or \
                     export_db['upstream ei type [-]'].ix[index] == 'wet-mate':
                      phase.op_ve[a].op_seq_sea[index].extend([log_op["Lower_Cable_Seabed"]])

                  elif export_db['upstream ei type [-]'].ix[index] == 'j-tube':
                      phase.op_ve[a].op_seq_sea[index].extend([log_op["Jtube_Connect"]])

                  else:
                      msg = ("Error in export cable {}: "
                ...
                elif export_db['upstream termination type [-]'].ix[index] == 'collection point':

                    cp_id = export_db['upstream termination id [-]'].ix[index]

                    if export_db['upstream ei type [-]'].ix[index] == 'hard-wired':
                        phase.op_ve[a].op_seq_sea[index].extend([log_op["Lower_CP_Seabed"]])

                    elif export_db['upstream ei type [-]'].ix[index] == 'dry-mate' or \
                         export_db['upstream ei type [-]'].ix[index] == 'wet-mate' or \
                         export_db['upstream ei type [-]'].ix[index] == 'j-tube':

                        if cp_db['type [-]'].ix[cp_id] == 'seabed' or \
                           cp_db['type [-]'].ix[cp_id] == 'seabed with pigtails': 

                            if export_db['upstream ei type [-]'].ix[index] == 'dry-mate':
                                phase.op_ve[a].op_seq_sea[index].extend([log_op["Lower_Cable_Seabed"]])

                            elif export_db['upstream ei type [-]'].ix[index] == 'wet-mate':
                            ...
                            msg = ("Error in export cable: Collection point "
                        ...
                        msg = ("Error in export cable {}: Upstream electrical "
                               "interface type not recognized".format(index))
                        module_logger.warning(msg)
