...
def sched_m_direct(seq, ind_sol, install, log_phase, site, entry_point, device, layout, foundation, penet_rates,
                  sched_sol):
    ...
    elem_area = direct_db['dry mass [kg]'].fillna(0)/1000.0 # [ton]
    elem_mass = direct_db['length [m]'].fillna(0) * direct_db['width [m]'].fillna(0)
...
#                    nb_op_sea = len(log_phase.op_ve[seq].op_seq_sea[dev_id])
        ...
        UTM_elem = [direct_db['x coord [m]'].ix[id_first_elem],
                    direct_db['y coord [m]'].ix[id_first_elem],
                    direct_db['zone [-]'].ix[id_first_elem]]
            ...
            for log_op_sea in log_phase.op_ve[seq].op_seq_sea[elem_id]:  # loop over the nb of offshore logistic operations
                ...
                ####################################
                    ...
                    if log_op_sea.time_function == "distance":
                        ...
                        UTM_elem = [direct_db['x coord [m]'].ix[elem_id],
                                    direct_db['y coord [m]'].ix[elem_id],
                                    direct_db['zone [-]'].ix[elem_id]]  
                        ...
                        if  elem_ix == last_elem_ix:
                            ...
                            UTM_next_elem = [direct_db['x coord [m]'].ix[next_elem_id],
                                             direct_db['y coord [m]'].ix[next_elem_id],
                                             direct_db['zone [-]'].ix[next_elem_id] ]                              
                            # compute distance from last element to the lease area entry point
                    ...
                    elif log_op_sea.description == "Lowering anchors with mooring lines + Direct-embedment anchor seafloor penetration through suction-embedment + Tensioning" or \
                         log_op_sea.description == "Lowering anchors with mooring lines + Direct-embedment anchor seafloor penetration through jetting-embedment + Tensioning" or \
                         log_op_sea.description == "Lowering anchors with mooring lines + Direct-embedment anchor seafloor penetration through mechanical-embedment + Tensioning":
                        ...
                        UTM_elem_x = direct_db['x coord [m]'].ix[elem_id]
                        UTM_elem_y = direct_db['y coord [m]'].ix[elem_id]
                        UTM_zone = direct_db['zone [-]'].ix[elem_id]
                        ...
                        depth_of_ins = direct_db['installation depth [m]'].ix[elem_id]


                        if log_op_sea.description == "Lowering anchors with mooring lines + Direct-embedment anchor seafloor penetration through suction-embedment + Tensioning":
                            penetration_rate = penet_rates.ix['ROV with suction pump [m/h]'][soil_type]
                        elif log_op_sea.description == "Lowering anchors with mooring lines + Direct-embedment anchor seafloor penetration through jetting-embedment + Tensioning":
                            penetration_rate = penet_rates.ix['ROV with jetting [m/h]'][soil_type]
                        elif log_op_sea.description == "Lowering anchors with mooring lines + Direct-embedment anchor seafloor penetration through mechanical-embedment + Tensioning":
                            penetration_rate = penet_rates.ix['Hammer [m/h]'][soil_type]
                        else:
                            msg = ("Unknown direct anchor installation method "
                                   "penetration rate for phase {}.".format(
                                   log_op_sea.description))
                            module_logger.warning(msg)
                            penetration_rate = 0
