...
def sched_s_struct(seq, ind_sol, install, log_phase, site, entry_point, device, sub_device,
                   layout, sched_sol):
    """
    sched_dev_deck determines the duration of each individual logistic
    operations for the installtion of ocean energy devices
    ...
    elem_area = {}
    elem_mass = {}
    for index, row in device_db.iterrows(): # loop over the number of devices
        # extract the mass length and total mass of all elements
        anchor_mass = support_db['dry mass [kg]']/1000.0 # [ton]
        anchor_lenght = support_db['length [m]'] # [m]
        anchor_width = support_db['width [m]'] # [m]
        # obtain the dry mass and dimensions of all elements
        elem_area[index] = anchor_lenght*anchor_width # [m^2]
        elem_mass[index] = anchor_mass # [ton]
    ...
    # if numpy.isnan(deck_cargo):
    #     deck_cargo = 0.0
    # if numpy.isnan(deck_area):
    #     deck_area = 0.0
...
#                    nb_op_sea = len(log_phase.op_ve[seq].op_seq_sea[dev_id])
        ...
        UTM_elem = [device_db['x coord [m]'].ix[id_first_elem],
                    device_db['y coord [m]'].ix[id_first_elem],
                    device_db['zone [-]'].ix[id_first_elem]]
            ...
            for log_op_sea in log_phase.op_ve[seq].op_seq_sea[elem_id]: 
                    ...
                    if log_op_sea.description == "Vessel Positioning": # vessel positioning
                        if any(olc_method[op] == "vessel" for op in range(len(olc_method))):
                            if ves_type == "JUP Barge" or ves_type == "JUP Vessel":
                                ...
                                olc_trans = nan2zero(olc_trans)
                                # SPEED:
                                UTM_elem_x = support_db['x coord [m]'].ix[elem_id]
                ...
                ####################################
                    ...
                    if log_op_sea.time_function == "distance":
                        ...
                        UTM_elem = [device_db['x coord [m]'].ix[elem_id],
                                    device_db['y coord [m]'].ix[elem_id],
                                    device_db['zone [-]'].ix[elem_id]]
                        ...
                        # compute the distance from element to element
                            ...
                            UTM_next_elem = [device_db['x coord [m]'].ix[next_elem_id],
                                             device_db['y coord [m]'].ix[next_elem_id],
                                             device_db['zone [-]'].ix[next_elem_id] ]     
                            # compute distance from last element to the lease area entry point
