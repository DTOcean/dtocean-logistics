...
def sched_gravity(seq, ind_sol, install, log_phase, site, entry_point, device, layout, foundation,
                  sched_sol):
...
#                    nb_op_sea = len(log_phase.op_ve[seq].op_seq_sea[dev_id])
        ...
        UTM_elem = [gravity_db['x coord [m]'].ix[id_first_elem],
                    gravity_db['y coord [m]'].ix[id_first_elem],
                    gravity_db['zone [-]'].ix[id_first_elem]]
        ...
        for elem_id in id_el_journey[jour]:
            ...
            for log_op_sea in log_phase.op_ve[seq].op_seq_sea[elem_id]: 
                ...
                if not pd.isnull(time_method[0]): 
                    ...
                    if log_op_sea.description == "Vessel Positioning": # vessel positioning
                        if any(olc_method[op] == "vessel" for op in range(len(olc_method))):
                            if ves_type == "JUP Barge" or ves_type == "JUP Vessel":
                                ...
                                olc_trans = nan2zero(olc_trans)
                                # SPEED:
                                UTM_elem_x = gravity_db['x coord [m]'].ix[elem_id]
                                UTM_elem_y = gravity_db['y coord [m]'].ix[elem_id]
                                UTM_zone = gravity_db['zone [-]'].ix[elem_id]
                    ...
                    elif log_op_sea.description == "Gravity based foundation or anchor positioning":
                        if ve_combi[0][2].ix['Vessel type [-]'] == "JUP Barge" or ve_combi[0][2].ix['Vessel type [-]'] == "JUP Vessel":
                            olc_Hs = 0
                        elif any(olc_method[op] == "vessel" for op in range(len(olc_method))):
                            olc_Hs = ve_combi[0][2].ix['OLC: Transit maxHs [m]']
                            olc_Tp = ve_combi[0][2].ix['OLC: Transit maxTp [s]']
                            olc_Ws = ve_combi[0][2].ix['OLC: Transit maxWs [m/s]']
                            olc_Cs = ve_combi[0][2].ix['OLC: Transit maxCs [m/s]']
                ...
                ####################################
                    ...
                    if log_op_sea.time_function == "distance":
                        ...
                        UTM_elem = [gravity_db['x coord [m]'].ix[elem_id],
                                    gravity_db['y coord [m]'].ix[elem_id],
                                    gravity_db['zone [-]'].ix[elem_id]]  
                        ...
                        if  elem_ix == last_elem_ix:
                            ...
                            UTM_next_elem = [gravity_db['x coord [m]'].ix[next_elem_id],
                                             gravity_db['y coord [m]'].ix[next_elem_id],
                                             gravity_db['zone [-]'].ix[next_elem_id] ]                              
                            # compute distance from last element to the lease area entry point
                    ...
                    # type of function 
                    elif log_op_sea.time_function == "lowering":
                        # extract the coordinates of the current element being installed
                        UTM_elem_x = gravity_db['x coord [m]'].ix[elem_id]
                        UTM_elem_y = gravity_db['y coord [m]'].ix[elem_id]
                        UTM_zone = gravity_db['zone [-]'].ix[elem_id]
                        ...
                        # obtain operation limit conditions for the operation                  
                        if ve_combi[0][2].ix['Vessel type [-]'] == "JUP Barge" or ve_combi[0][2].ix['Vessel type [-]'] == "JUP Vessel":
                            olc_Hs = 0
                        else:
                            olc_Hs = ve_combi[0][2].ix['OLC: Transit maxHs [m]']
                        ...
                        olc = nan2zero(olc)
                        # compute lowering rate
                        area_vertical_m2 = max(gravity_db['width [m]'].ix[elem_id] * gravity_db['height [m]'].ix[elem_id], \
                                               gravity_db['length [m]'].ix[elem_id] * gravity_db['height [m]'].ix[elem_id])                                               
                        weight_kgs = gravity_db['dry mass [kg]'].ix[elem_id]
                        op_speed_max_m_s = weight_kgs/(66.0*area_vertical_m2)
                        op_speed_max_m_h = op_speed_max_m_s*(60.0*60.0)
                        lowering_rate = 0.6*op_speed_max_m_h
                        # compute lowering time                                   
                        op_time = ins_depth/lowering_rate

                        op_id_sea.append(log_op_sea.description)                    
                        op_dur_sea.append(op_time)
                        op_olc_sea.append(olc)
    
                        op_id_sea_jour[jour].append(log_op_sea.description)
                        op_dur_sea_jour[jour].append(op_time)
                        op_olc_jour[jour].append(olc)
                        
                        journey[jour]['sea_id'].append(log_op_sea.description)
                        journey[jour]['sea_dur'].append(op_time)
                        journey[jour]['sea_olc'].append(olc)
                    else:
