...
def sched_driven(seq, ind_sol, install, log_phase, site, entry_point, device, foundation, penet_rates, other_rates,
                  sched_sol):
    ...
    elem_area = driven_db['dry mass [kg]'].fillna(0)/1000.0 # [ton]
    elem_mass = driven_db['length [m]'].fillna(0) * driven_db['width [m]'].fillna(0)
...
#                    nb_op_sea = len(log_phase.op_ve[seq].op_seq_sea[dev_id])
        ...
        UTM_elem = [driven_db['x coord [m]'].ix[id_first_elem],
                    driven_db['y coord [m]'].ix[id_first_elem],
                    driven_db['zone [-]'].ix[id_first_elem]]
        ...
        for elem_id in id_el_journey[jour]:
            ...
            for log_op_sea in log_phase.op_ve[seq].op_seq_sea[elem_id]:  # loop over the nb of offshore logistic operations
                ...
                # discriminate between the time assessment methods
                if pd.isnull(time_method[0])==False and log_op_sea.time_function!="grouting": # default value
                    ...
                    if log_op_sea.description == "Vessel Positioning": # vessel positioning
                        if any(olc_method[op] == "vessel" for op in range(len(olc_method))):
                            if ves_type == "JUP Barge" or ves_type == "JUP Vessel":
                                ...
                                olc_trans = nan2zero(olc_trans)
                                # SPEED:
                                UTM_elem_x = driven_db['x coord [m]'].ix[elem_id]
                                UTM_elem_y = driven_db['y coord [m]'].ix[elem_id]
                                UTM_zone = driven_db['zone [-]'].ix[elem_id]
                                ...
                                site_coord = site[ (site['x coord [m]'] == float( closest_point[0] )) & \
                                                   ...
                                                   (site['zone [-]'] == UTM_zone) ]
                                location_depth = site_coord['bathymetry [m]'].iloc[0]
                    ...
                    elif log_op_sea.description == "Guiding template positioning + seafloor preparation equipment preparation" or \
                         log_op_sea.description == "Seafloor preparation + support structure positioning and equipment preparation":
                        ...
                        olc_Tp = olc_method[1]
                        olc_Ws = olc_method[2]
                        if ve_combi[0][2].ix['DP [-]'] >0:
                            olc_Cs = olc_method[3]*2
                        else:
                            olc_Cs = olc_method[3]
                        olc_trans = [olc_Hs, olc_Tp, olc_Ws, olc_Cs]
                        olc_trans = nan2zero(olc_trans)
                    ...
                    elif log_op_sea.description == "Grouting equipment removal":
                ...
                ####################################
                    ...
                    if log_op_sea.time_function == "distance":
                        ...
                        UTM_elem = [driven_db['x coord [m]'].ix[elem_id],
                                    driven_db['y coord [m]'].ix[elem_id],
                                    driven_db['zone [-]'].ix[elem_id]]  
                        ...
                        if  elem_ix == last_elem_ix:
                            ...
                            UTM_next_elem = [driven_db['x coord [m]'].ix[next_elem_id],
                                             driven_db['y coord [m]'].ix[next_elem_id],
                                             driven_db['zone [-]'].ix[next_elem_id] ]                              
                            # compute distance from last element to the lease area entry point
                    ...
                    elif log_op_sea.time_function == "penetration_time":
                        if ve_combi[0][2].ix['Vessel type [-]'] == "JUP Barge" or ve_combi[0][2].ix['Vessel type [-]'] == "JUP Vessel":
                            olc_Hs = 0
                            olc_Cs = 0
                        else:
                        ...
                        depth_of_ins = driven_db['installation depth [m]'].ix[elem_id]


                        if log_op_sea.description == "Driven pile foundation or anchor seafloor penetration through drilling rig + positioning":
                            penetration_rate = penet_rates.ix['Drilling rig [m/h]'][soil_type]
                        elif log_op_sea.description == "Driven pile foundation or anchor seafloor penetration through hammering + positioning":
                            penetration_rate = penet_rates.ix['Hammer [m/h]'][soil_type]
                        elif log_op_sea.description == "Driven pile foundation or anchor seafloor penetration through vibro-driving + positioning":
                            penetration_rate = penet_rates.ix['Vibro driver [m/h]'][soil_type]
                        else:
                    ...
                    elif log_op_sea.time_function == "grouting":
                        if ve_combi[0][2].ix['Vessel type [-]'] == "JUP Barge" or ve_combi[0][2].ix['Vessel type [-]'] == "JUP Vessel":
                            olc_Hs = 0
                        else:
                            olc_Hs = ve_combi[0][2].ix['OLC: Transit maxHs [m]']
                        ...
                        op_olc_sea.append(olc)
                        op_time = []
                        grout_vol = driven_db['grout volume [m3]'].ix[elem_id]
                        grout_rate = float(other_rates.ix['Grout rate [m3/h]'].values[0])
                        op_time.append( log_op_sea.time_value + grout_vol/grout_rate )
                        op_time_total = sum(op_time)
