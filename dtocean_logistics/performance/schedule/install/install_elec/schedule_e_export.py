...
def sched_e_export(seq, ind_sol, install, log_phase, site, entry_point,
    ...
    nb_cables = len(export_db)
    # Extract trenching technique
    strategy = log_phase.op_ve[seq].description
    if strategy == 'Static Export Cable Installation Strategy: ploughing':
        trench_type = 'Ploughing [m/h]'
    elif strategy == 'Static Export Cable Installation Strategy: jetting':
        trench_type = 'Jetting [m/h]'
    elif strategy == 'Static Export Cable Installation Strategy: cutting':
        trench_type = 'Cutting [m/h]'
    elif strategy == 'Static Export Cable Installation Strategy: dredging':
        trench_type = 'Ploughing [m/h]'
    elif strategy == 'Static Export Cable Installation Strategy: no data':
        trench_type = 'Ploughing [m/h]'
    ...
    elem_length = export_db['length [m]'].fillna(0)
    elem_mass = elem_length*export_db['dry mass [kg/m]'].fillna(0)/1000.0 
    ...
    if sol_pd_series['Vessel type [-]'] == 'CLV':
        ...
        msg = ("Installation vessel type {} not recognized. Allowed: CLV or "
               "CLB.".format(sol_pd_series['Vessel type [-]']))
        ...
        for op_prep in range(nb_op_prep): # loop over the nb of onshore logistic operations
            ...
            if not pd.isnull(time_method[0]): # direct value
                ...
                if log_op_prep.time_function == "load_cable":   
                    ...
                    jour_array_db = export_db.ix[id_el_journey[jour]]
                    ...
                    if log_op_sea.description == "Vessel Positioning": # vessel positioning
                        if any(olc_method[op] == "vessel" for op in range(len(olc_method))):
                            if ves_type == "JUP Barge" or ves_type == "JUP Vessel":
                                ...
                                olc_trans = nan2zero(olc_trans)
                                # SPEED:
                                UTM_elem = [ export_db['x coord [m]'].ix[elem_id],
                                             export_db['y coord [m]'].ix[elem_id],
                                             export_db['zone [-]'].ix[elem_id] ]
                ...
                ####################################                             
                    ...
                    if log_op_sea.time_function == "surface_time":
                        ...
                        if route_counter < (len(elem_route)-1):  # excludes the last element from the calculations                                   
                            ...
                            burial_rate = laying_rates[soil_type][trench_type] # [m/h]
                            # compute total lenght and route time                           
                            length = distance(UTM_ini,UTM_fin)*1000.0 # [m]
