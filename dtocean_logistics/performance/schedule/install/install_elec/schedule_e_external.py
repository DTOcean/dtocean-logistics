...
def sched_e_external(seq, ind_sol, install, log_phase, site, entry_point,
                      external_protection, sched_sol):
    """sched_external determines the duration of each individual logistic operations
    ...
    ext_prot_db = external_protection
    
    mattress_db = ext_prot_db[ext_prot_db['protection type [-]'] == 'concrete matress']
    rockbag_db = ext_prot_db[ext_prot_db['protection type [-]'] == 'rock filter bag']
    
    nb_ext_p = len(ext_prot_db)
    ...
    nb_eq_type = len(log_phase.op_ve[seq].sol[ind_sol]['VEs'][0])-3
    ...
    elem_eq_area = [] # external protection is equipment
    elem_eq_mass = [] # external protection is equipment
    
    # calculate the footprint and the weight due to the ROV equipment
    eq_area = []
    eq_cargo = []
    elem_eq_id = []
    for ind_eq_type in range(nb_eq_type):

        sol_pd_eq_series = log_phase.op_ve[seq].sol[ind_sol]['VEs'][0][ind_eq_type+3][2]
        sol_type_eq = log_phase.op_ve[seq].sol[ind_sol]['VEs'][0][ind_eq_type+3][0]
        num_eq_per_type = log_phase.op_ve[seq].sol[ind_sol]['VEs'][0][ind_eq_type+3][1]

        for ind_eq_i in range(num_eq_per_type):
            if sol_type_eq == 'rov':
...
#    deck_area = 100 # FOR TESTS, comment AFTER!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    
    ######################################################################################
    # calculate the footprint and the weight due to the Mattress and Rock Bag equipments #
    ######################################################################################
    mattress_area = []
    mattress_mass = []
    mattress_height = []
    ...
    # extract the quantity and characteristics of individual mattresses and rock bags from the solution
    for ind_eq_type in range(nb_eq_type):
        ...
        for ind_eq_i in range(num_eq_per_type):
            if sol_type_eq == 'mattress':
                mattress_area.append( sol_pd_eq_series.ix['Unit lenght [m]'] * sol_pd_eq_series.ix['Unit width [m]'] ) # external protection is equipment
                mattress_mass.append( sol_pd_eq_series.ix['Unit weight air [t]'] ) # external protection is equipment
                mattress_height.append( sol_pd_eq_series.ix['Unit thickness [m]'] ) # external protection is equipment                
            elif sol_type_eq == 'rock_filter_bags':
                rockbag_area.append( (math.pi/4) * sol_pd_eq_series.ix['Diameter [m]'] * sol_pd_eq_series.ix['Diameter [m]'] ) # external protection is equipment
                rockbag_mass.append( sol_pd_eq_series.ix['Weight [t]'] ) # external protection is equipment
                rockbag_height.append( sol_pd_eq_series.ix['Height [m]'] ) # external protection is equipment    

    # compute the quantity and characteristics of stacked mattresses
    stacked_mattress = np.cumsum(mattress_height)  # stack mattresses height   
    mattress_indice = indices(stacked_mattress, lambda x: x>2) # maximum of 2 meters height for a stack
    
    if not mattress_indice:  # if the mattress_indice is empty, all elements can be put in one stack
        nb_mattress_per_stack = len(stacked_mattress)
        nb_mattress_stacks = 1
        mattress_id = list(mattress_db.index[:])
    else:                    # if not the mattress_indice includes the number of mattresses in one stack
        ...
        for stack in range( int(nb_mattress_stacks) ):
            if stack < nb_mattress_stacks-1:
                mattress_id.append( list(mattress_db.index[from_elem:to_elem]) )
                from_elem = to_elem
                to_elem = to_elem + nb_mattress_per_stack
            else:
                from_elem = from_elem
                to_elem = to_elem + nb_mattress_last_stack
                mattress_id.append( list(mattress_db.index[from_elem:to_elem]) )
                
    matt_area_per_stack = mattress_area[0]
    matt_mass_per_stack = mattress_mass[0]*nb_mattress_per_stack

    # compute the quantity and characteristics of stacked rockbags
    stacked_rockbag = np.cumsum(rockbag_height)    # stack rockbag height
    rockbag_indice = indices(stacked_rockbag, lambda x: x>2) # maximum of 2 meters height for a stack

    if not rockbag_indice:  # if the rockbag_indice is empty, all elements can be put in one stack
        nb_rockbag_per_stack = len(stacked_rockbag)
        nb_rockbag_stacks = 1
        rockbag_id = list(rockbag_db.index[:])        
    else:                    # if not, the rockbag_indice includes the number of mattresses in one stack
        ...
        for stack in range( int(nb_rockbag_stacks) ):
            if stack < nb_rockbag_stacks-1:
                rockbag_id.append( list(rockbag_db.index[from_elem:to_elem]) )
                from_elem = to_elem
                to_elem = to_elem + nb_rockbag_per_stack
            else:
                from_elem = from_elem
                to_elem = to_elem + nb_rockbag_last_stack
                rockbag_id.append( list(rockbag_db.index[from_elem:to_elem]) )
                
    rockbag_area_per_stack = rockbag_area[0]
    rockbag_mass_per_stack = rockbag_mass[0]*nb_rockbag_per_stack
    
    # arrange all the stacks in one array
    elem_area = [matt_area_per_stack]*nb_mattress_stacks + [rockbag_area_per_stack]*nb_rockbag_stacks
    elem_mass = [matt_mass_per_stack]*nb_mattress_stacks + [rockbag_mass_per_stack]*nb_rockbag_stacks
    elem_id = mattress_id + rockbag_id
    # initialize variables
    nb_elem_port = len(elem_id) # initialise the number of elements to be transported that are initially at port
    ...
    while nb_elem_port > 0:
        ...
        # print 'nb_elem_port: ' + str(nb_elem_port)

        # reset variables  
        id_elem_area = []
        id_elem_mass = []  
        # determine the cumulative vector of element dry masses and area
        elem_area_accum = np.cumsum(elem_area)
        elem_mass_accum = np.cumsum(elem_mass)
        # determine the maximum number of elements that can fit on-deck due to max deck area or max deck cargo limitations
        nb_elem_area = indices(elem_area_accum, lambda x: x>deck_area)
        if not nb_elem_area:  # if the list is empty, all elements can be carried
            nb_elem_area = len(elem_area_accum)
            for a in range(nb_elem_area): # loop inside each stack that can be carried, to extract the id's of each element
                id_elem_area.append(elem_id[a]) 
            if a > 0:
                id_elem_area = list(chain(*id_elem_area)) # flatten the list of lists
        else:
        ...
        nb_elem_mass = indices(elem_mass_accum, lambda x: x>deck_cargo)
        if not nb_elem_mass: # if the list is empty, all elements can be carried
            nb_elem_mass = len(elem_mass_accum)
            for a in range(nb_elem_mass): # loop inside each stack that can be carried, to extract the id's of each element
                id_elem_mass.append(elem_id[a]) 
            if a > 0:
                id_elem_mass = list(chain(*id_elem_mass)) # flatten the list of lists
        else:
        ...
        if nb_el_journey[nb_journey] == nb_elem_port:
            ...
            msg = ("External protection cannot fit on the turntable.")
            ...
            elem_id = elem_id[nb_el_journey[-1]:]
    ...
    for jour in range(nb_journey):
        ...
        UTM_elem = [ext_prot_db['x coord [m]'].ix[id_first_elem],
                    ext_prot_db['y coord [m]'].ix[id_first_elem],
                    ext_prot_db['zone [-]'].ix[id_first_elem]]
                    ...
                    if log_op_sea.description == "Vessel Positioning": # vessel positioning
                        if any(olc_method[op] == "vessel" for op in range(len(olc_method))):
                            if ves_type == "JUP Barge" or ves_type == "JUP Vessel":
                                ...
                                olc_trans = nan2zero(olc_trans)
                                # SPEED:
                                UTM_elem = [ ext_prot_db['x coord [m]'].ix[elem_id],
                                             ext_prot_db['y coord [m]'].ix[elem_id],
                                             ext_prot_db['zone [-]'].ix[elem_id] ]
                ...
                ####################################
                    ...
                    if log_op_sea.time_function == "distance":
                        ...
                        # compute the distance from element to element
                            ...
                            UTM_next_elem = [ext_prot_db['x coord [m]'].ix[next_elem_id],
                                             ext_prot_db['y coord [m]'].ix[next_elem_id],
                                             ext_prot_db['zone [-]'].ix[next_elem_id] ]     
                            # compute distance from last element to the lease area entry point
