...
def simul_plot_bar(outputs, simul_time):

    mpl_fig = plt.figure()


    ax = mpl_fig.add_subplot(131)

    y = [outputs['opt_sol']['total_prep_time [h]'],
         outputs['opt_sol']['total_wait_time [h]'][0],
         outputs['opt_sol']['total_sea_time [h]']]
    N = 1
    ...
    ax = mpl_fig.add_subplot(132)

    y = [outputs['opt_sol']['total_port_cost [EUR]'],
         outputs['opt_sol']['total_vessel_cost [EUR]'],
         outputs['opt_sol']['total_eq_cost [EUR]']]
    N = 1
    ind = np.arange(N)    # the x locations for the groups
    ...
    log_phase_descript = log_phase_descript.replace("/","_")
    plt.savefig( 'Fig_Plot_' + log_phase_descript + '.png' )
    # plt.show()
...
def simul_plot_pie(outputs, simul_time):

    mpl_fig = plt.figure()

    ax = mpl_fig.add_subplot(131)
    labels = 'prep time', 'wait time', 'sea time'
    values = [ 100.0*outputs['opt_sol']['total_prep_time [h]']/max(outputs['opt_sol']['total_time [h]']),
         100.0*outputs['opt_sol']['total_wait_time [h]'][0]/max(outputs['opt_sol']['total_time [h]']),
         100.0*outputs['opt_sol']['total_sea_time [h]']/max(outputs['opt_sol']['total_time [h]']) ]
    ...
    values = [100.0*outputs['opt_sol']['total_port_cost [EUR]']/outputs['opt_sol']['total_cost [EUR]'],
         100.0*outputs['opt_sol']['total_vessel_cost [EUR]']/outputs['opt_sol']['total_cost [EUR]'],
         100.0*outputs['opt_sol']['total_eq_cost [EUR]']/outputs['opt_sol']['total_cost [EUR]'] ]#,
         # 100.0*outputs['opt_sol']['total_fuel_cost [EUR]']/outputs['opt_sol']['total_cost [EUR]'] ]
    colors = ['yellowgreen', 'mediumpurple', 'lightskyblue']
    explode = (0.075, 0.025, 0.025)    # proportion with which to offset each wedge
    ...
    log_phase_descript = log_phase_descript.replace(" ","_")
    log_phase_descript = log_phase_descript.replace("/","_")
    plt.savefig( 'Fig_Pie_' + log_phase_descript + '.png' )
    # plt.show()
...
def simul_plot_tables(module, outputs, simul_time):
    ...
    log_phase_descript = log_phase_descript.replace(" ","_")
    log_phase_descript = log_phase_descript.replace("/","_")
    plt.savefig( 'Fig_Table_' + log_phase_descript + '.png' )
    plt.show()
...
def out_ploting(module, outputs, simul_time, PLOT_FLAG):

    if PLOT_FLAG:
        simul_plot_bar(outputs, simul_time)
        simul_plot_pie(outputs, simul_time)
        simul_plot_tables(module, outputs, simul_time)
...
def out_ploting_installation(Installation):

    # Data
    num_phases = len(Installation['inst_log'])
    pos = arange(0.5,(num_phases)/2 + 1.0,0.5)

    ylabels = []
    customDates = []
    for index in range(num_phases):
        l_phase = Installation['inst_log'][index]
        log_phase_descript = l_phase
        log_phase_descript = log_phase_descript.replace("mooring systems with","")
        # log_phase_descript = log_phase_descript.replace(" ","_")
        # log_phase_descript = log_phase_descript.replace("/","_")
        ylabels.append(log_phase_descript)
        start_dt = Installation['inst_sol'][l_phase]['opt_sol']['date']['start_dt']
        prep_dt = Installation['inst_sol'][l_phase]['opt_sol']['date']['start_dt'] + timedelta(hours = sum(Installation['inst_sol'][l_phase]['opt_sol']['time']['total_prep_time [h]']))
        depart_dt = Installation['inst_sol'][l_phase]['opt_sol']['date']['depart_dt']
        end_dt = Installation['inst_sol'][l_phase]['opt_sol']['date']['end_dt']
    ...
    plt.savefig( 'Fig_Gantt_Installation' + '.png' )
